...
if ($result = mysqli_query($link, "SELECT * FROM cad_funcionario WHERE nome LIKE '$log'  and  senha = '$pass'")){
        while($user = mysqli_fetch_assoc($result)){
			
		if ($log == $user['nome'] || $pass == $user['senha']){
			
		setcookie('funcionario', $user['nome'], (time() + (6 * 3600)));	
			
		echo '<script>
		
		<!-- window.alert("Você foi autorizado..!\r"); -->
